...
class SimplexSolver():
    ...
    def run_simplex(self, A, b, c, prob='max', ineq=[],
        ...
        while (not self.should_terminate()):
            ...
            if pivot[1] < 0:
                if (enable_msg):
                    print ("Тут не існує не від'ємної опорної точки. "
                           "Рішення знайти неможливо.")
                self.infeasible_doc()
                self.print_doc()
                return None
            else:
                ...
                if (enable_msg):
                    clear()
                    self._print_tableau()
                    print(("\nПрисутній негативний елемент в нижньому рядку, "
                          "отже, дане рішення не оптимальне. "
                          "Знайдемо опорну точку."
                          "введена змінна %s і змінна, яку ми "
                          "прибираємо %s.\n" %
                           (str(self.entering[pivot[0]]),
                           str(self.departing[pivot[1]]))))
                    self._prompt()
                    print("\nПроводимо операцію з рядками доки "
                          "опорна точка не дорівнюватиме 1 і усі інші елементи не "
                          "дорівнюватимуть 0.\n")
        ...
        if (enable_msg):
            ...
            self._print_tableau()
            print(("Поточне рішення: %s\n" % str(solution)))
            print("Рішення знайдене! Ви можете завантажити рішення в .tex форматі")
        self.print_doc()
        return solution
        ...
        if (self.prob == 'min'):
            ...
            for key in list(solution.keys()):
                ...
                if key.startswith('x'):
                    old_key = key
                    key = key.replace('x', 'y')
                    solution[key] = solution.pop(old_key)
                    solution[key] = 0 if solution[key] == Fraction(0, 1) else solution[key]
    ...
    def init_problem_doc(self):
        ...
        for index, x in enumerate(self.c):
            ...
            if self.prob == 'max':
                ...
                func += (r"%s %sy_%s " % (opp, str(x), str(index + 1)))
            found_value = True
        self.doc += (r"\%s{%s} \\ "
                     r"\end{equation*}" % (self.prob, func))
